...
活俠傳同人遊戲 — Roguelike 牌組構築 ＋ 割草無雙。核心戰鬥（合成連鎖、境界連段、割草敵陣、附魔）已成形；
**里程碑 3「江湖遠征」run 結構 Phase 1–5 全上線**（run loop、拉霸、客棧、遺物、奇遇、主角屬性、跨 run 據點 meta）。
...
| [docs/systems/merge.md](docs/systems/merge.md) | 合成規則 ＋ 附魔（含「卡自身效果 vs 附魔」）。 |
| [docs/systems/combat.md](docs/systems/combat.md) | 割草戰鬥：敵陣、招式鎖定、擊退、回合流程。 |
| [docs/systems/combo.md](docs/systems/combo.md) | 連段怎麼加成（攻擊牌加次數、功能牌加法）。 |
