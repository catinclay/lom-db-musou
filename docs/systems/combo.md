...
依境界數字由小到大遞增出牌就累積連段（`ComboTracker`，step 從 1 起）。連段對效果的加成**分三類**：

- **攻擊／純狀態牌 ＝ 加「次數」**（不是加每發傷害或每次層數）。預設 `comboScale` 把 `effect.hits` 乘上 step：
  - 暗器：3 → 6 → 9 發（散射多釘幾人）。
  - 劈砍 / 貫 / 崩山：1 → 2 → 3 **波**，每波在 `combat.js` **重新選一次目標**再打
    （劈砍重選最近排、貫重選最人多的一路），所以動畫也會**演多次**（劈砍兩次）。
  - 崩山的**擊退**也逐波施加（`resolveAttack` 收 `knockback`，打完一波推一波）。
  - 毒霧：1 → 2 → 3 波，每波重新覆蓋近三排並各上一份境界縮放後的毒，所以動畫也會確實跑兩／三次。
  - 火藥：1 → 2 → 3 波；第一擊維持舊鎖定，後續擊必換成不同 3×3，優先命中較多敵人、盡量不炸空位。
  - 每發標 `wave`，`FormationView` 依 `tuning.anim.combatWaveDelay` 錯開每波演出；擊退另用 `knockbackWaveDelay`。
...
`statusStacks` 只吃境界曲線；連段靠多波重複施加，所以總層數可以相同，但鎖定與動畫會逐次發生。
