...
> 相關：[combo](combo.md)（連擊＝次數）、[status](status.md)（中毒/燃燒的 tick）、[merge](merge.md)（階級與忘形）。
...
- **招式鎖定**（`combat.TARGET`，卡的 `target`）：
  ...
  - `BLAST`（火藥）：一個 `area.size`×`area.size` 方塊。第一擊沿用「必含最近排、涵蓋最多人」；連擊後續擊會改選不同方塊，優先命中最多敵人並盡量不炸空位，營造分散爆炸。**預留之後開放玩家指定中心**。
  - `SCATTER`（暗器）：每根隨機釘最前排一人。
  - `SINGLE` / `MULTI` / `RANDOM`：已實作，暫無卡用。
  - **連擊＝次數**：`effect.hits` 隨連擊變多，攻擊與純狀態範圍牌都會**重打幾波**，每波重新選目標並標 `wave`。UI 依 `tuning.anim.combatWaveDelay` 錯開動畫；崩山的擊退也逐波施加。詳見 [combo](combo.md)。
- **目前牌組**：攻擊 = 橫劈（整列）、貫（縱列）、崩山（整列＋擊退）、暗器（散射）、
  毒霧（近三排上中毒）、火藥（3×3 上燃燒）；技能 = 運氣調息（回內力）、臨機應變（獲得靈感）、忘形（境界歸零／升階）。
  防禦（護甲）機制還在，暫無卡產生護甲。
- **上狀態的卡**：`playCard` 對命中且存活的敵人套卡自身 `effectStatus`（毒霧／火藥，每波層數隨階級曲線成長，連擊增加波數）。
...
- **內力刻度**：core 以小格儲存，`3` 小格組成 UI 的 `1` 個完整氣輪。基礎每回合 `9` 小格；舊制一費／二費牌分別消耗 `3／6` 小格。卡面與 HUD 用同一組大小格符號，不再讓玩家同時消化費用數字與階級數字。
- **靈感**：每滿 `3` 點立即抽一張，餘數跨回合保留但每場重置。合成與忘形升階各給 `2` 點；臨機應變也會依階級與連擊產生靈感。
- **靈感演出**：左下靈感依 transcript 一顆一顆點亮；第三顆只花與普通一顆相同的點亮時間，隨即清空並繼續下一輪。「靈感滿溢」特效在背景自行淡出，不會被 await；抽牌飛行、滿格特效、後續靈感與合成都可並行。若飛行中的牌成為合成材料，合成 tween 直接接管它，不等待歸位。靈感同樣吃連鎖加速，動畫數值在 `tuning.anim.inspiration*`。
- **連擊提示降噪**：境界為 0 時不替手牌顯示續擊綠框；連擊為 0 時右上角完全隱藏連擊文字。建立第一段後才顯示可續擊牌與連擊數。
- **清場選擇**：玩家出牌清空敵陣且仍有補充波時，立即獲得 1 格（3 小格）內力並抽 1 張，畫面顯示「再來啊！」。按它會把**當前波剩餘排數**一次送入；也可照常結束回合。若 DoT 在按下結束回合後才清場，獎勵延到下個玩家回合，避免抽到的牌立即被棄掉。
